...
复盘“提示词 + 参考资产 + 成片”时，先找偏差**最早在哪里可见**：原始身份／场景参考 → 新生成或编辑的关键帧等视频资产 → 实际提交的图号顺序与提示词 → 成片。用户后来补充的实际输入和原文件优先于界面预览或事后记忆。

若错误已经出现在提交的视频资产中，先标记为上游资产输入问题，不直接归咎于视频提示词；若只在成片出现，再检查提示词歧义、参考冲突和模型补全。最早出现位置能缩小排查范围，但单次样本不能证明唯一因果。下一轮尽量只修一个已定位变量，保留原输入与结果，比较后再调整方法。
