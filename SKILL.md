...
3. 做轻量资产检查：角色身份是否已有；当前地点若是独特世界场景或剧情依赖场景结构，是否已有可用 Scene Plate；若缺失，先交接 Asset Skill，不要用最终 Prompt 临时发明关键场景结构。
4. 用一句话说明观众在这段结束时比开头多知道或多感受到什么。
5. 选择最小生成单位：单一连续镜头，或数个可独立重做的片段。
6. 多镜时先列共享锚点和镜头职能；单镜时直接确定起幅、过程与落幅。
7. 写最终提示词，只保留目标模型真正需要执行的内容。
8. 检查可见性、动作因果、实体数量、参考图职责、中心事件承载物、空间方向、时长容量与结束状态。
