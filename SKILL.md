...
1. **先理解需求。** 判断用户现在要讨论理论、确定试片方向、准备资产、写视频提示词，还是诊断已有成片；核对用户原话、已有材料、时长／比例、明确禁止与当前授权。把已确认事实、可撤回的执行假设、会改变故事的开放决定分开。缺失信息只有会改变身份、核心事件、镜头方向或结尾时才提问。
2. **确定最小故事方向。** 已有获批 Story Package 时忠实承接；用户授权探索非正史试片时，可依据已确认的人物与世界事实提出一段标明“试片假设”的简短因果线：人物目标 → 触发事件 → 可见反应 → 行动结果。不要把试片设定写成 Canon，也不要在用户已给出剧情时重新发明剧情。
