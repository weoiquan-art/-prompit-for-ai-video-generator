...
最终视频 Prompt 之前必须先做一次**轻量资产检查**。这不是要求每镜都新增资产，而是确认当前镜头真正依赖的角色、场景与特殊视觉事实是否已有可用载体。
...
先确认已有 Scene Plate 或首帧场景参考是否清楚负责该结构；不足时补最小的干净场景资产。
...
4. **判断最小资产并交接。** 检查已有身份与场景资料；只为必须锁定的视觉事实安排资产，逐张写清职责、不继承项与验收点。确需制作时交给 $jin-gpt-image-director；视频 Skill 负责“需要什么”，资产 Skill 负责“怎样生成／编辑”。没有实际文件时，只交付需求单，不声称资产已就绪。
...
6. **基于已验收资产写视频 Prompt。** 按实际上传顺序逐图分工，写清同一场景的锚点、起幅 → 行动／反应 → 结果、切镜后的状态继承；声音和少量失败保险最后加入。若资产未完成，先停在资产交接，不把计划中的图片当作已提交参考。
